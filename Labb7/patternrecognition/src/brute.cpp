// Labb 7, Pattern Recognition - Matheus Bernat (matvi959) & Caspian Süsskind (cassu286)
...
int main(int argc, char *argv[]) {
    ...
    // The code down here implements the brute algorithm:
/*
    for (int i = 0 ; i < N-3 ; ++i) {
    ...
    /* ------ The following code implements the sorting solution, called fast -------- */
    double slope;
    map<double,vector<Point>> myMap;
    for (int i = 0 ; i < N ; i++) {
        for(int j = 0; j < N; j++) {
            slope = points[i].slopeTo(points[j]);
            myMap[slope].push_back(points[j]);
        }
        for(std::pair<double, vector<Point>> element : myMap) {
            if(element.second.size() >= 3) {
                render_line(scene, points[i], element.second.back());
                a.processEvents();
            }
        }
        myMap.clear();
